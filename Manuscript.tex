...
Using data from 11 software projects,  XTREE found   larger reductions than three
...
engineering, we may be asking questions for which there is no currently acceptable
...
of Siegmund et al.~\cite{sven12} is certainly state of the art, it does not yet represent ``internationally accept best practices'' since that work is so
recent. 
...
To assess our planning methods, we use data from Jureczko et al.'s object-oriented JAVA systems~\cite{jureczko10}  and
  software system   configuration data from by  Siegmund et al.~\cite{sven12}\footnote{ See \url{http://git.io/vGYxc} for full access to this data.}.
 
  
  
   The Jureczko data records number of known defects for each class using a post-release bug tracking system. The classes are described in terms of
...
\input{tex/prediction_stats.tex}
  ...
  Our evaluation strategy (discussed below) divides this data into a training and a test set.
...
Therefore, we can be very confident that the predictors from the Siegmund data can assess
...
1 &         CD+FS &    49  &  13 & \quart{7}{25}{17}{115} \\\hline  
2 &      CD &    59  &  18 & \quart{15}{34}{36}{115} \\
2 &          BIC &    60  &  12 & \quart{24}{24}{38}{115} \\\hline  
3 &      XTREE &    62  &  6 & \quart{36}{12}{42}{115}  \\\hline \end{tabular}}
\end{center}
