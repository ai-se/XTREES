...
This presents such a tool, called XTREE and evaluates it against three alternatives.
In our experiments, XTREE reduced
...
The contributions of this paper are two-fold. Firstly, we offer
   XTREE, which has not been previously published.
   XTREE     works  on table of data with weighted class values indicating what rows are ``good'' and what rows are ``bad''.
XTREE returns plans that are changes to feature values in the ``bad'' rows to make them more ``good''.
Secondly, an evaluation strategy used to comparatively assess planners. As  shown below, that strategy less us identify  inferior planners. For example, when exploring software runtimes and software defects,
we show that XTREE is significantly better than the methods proposed in two of our prior publications~\cite{me12c}.
...
\ei  
Accordingly, we should use  {\em trust-increasing} planners. That is, we should use planners that push changde examples {\em towards} the
...
(Aside: If the reader is concerned that this 2d projection
...
issue is explored further in the \label{sect:tesd}.
\item
...
As an historical note,  Method\#2 was previously proposed in one of our prior publications~\cite{me12c}.
...
\subsection{Method4 =  XTREE =\\ Deltas in Decision Tree Branches}
...
Accordingly, XTREE uses a supervised method to generate its plans.
...
\ee
The last question can be answered by finding the deltas in branches of the decision tree that lead to {\em desired} from {\em current}. For full algorithmic details, refer to \fig{XTREE_bare}.
...
XTREE performs well in terms of  effectiveness. 
\fig{jur} and \fig{conf} show the effectiveness of our methods seen in 40 repeats with each data set.
...
occurred in the {\em train} phase of \fig{work}.
\ei
The results show planning can be very effective-- some of the expected value of the reductions are very large:
\bi
\item
Down to 22\% of the original baseline in Ant of \fig{jur};
\item
Down to 22\% of the original baseline in X264  of \fig{conf};
\ei
In all these results,  with one exception, XTREE always is the top-ranked method and with a median improvement at least 10\% lower (i.e. better) than the next ranked method. The  exception is SQL which (as shown in \fig{conf}), seems to defy all methods for optimization
so XTREE can only reduce the expected values of the runtimes by 1\%.

From these results, measured in terms of effectiveness, we recommend supervised-based division of the data (as used in XTREE) over unsupervised clustering
(as used in the other methods). As to the very simple methods (CD and CD+FSS), these were proposed in prior publications
on this work~\cite{me12c}. We now   observe  that that prior work was poorly evaluated so the recommendations
of that prior paper  must now be rejected.


As to succinctness , that is discussed in the next section.
...
The top plot of that figure reports results from one of the  Jureczko data sets ({\em lucene}) and the bottom
plot shows a Siegmund data set ({\em BDBJ}). 
