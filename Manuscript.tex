...
In the case studies of this paper  our evaluations shows
 ...
 where ``better'' means:
\bi
\item {\em Effective:}    i.e.  if  its recommendations  are  applied  then  some  statistically significant  change  in expected values of the results should  be  observed. 
\item {\em Succinct:} i.e. given two plans with the same effectiveness, we prefer the shorter one.
Such shorter plans are easier to explain (less to show) and faster to implement (less to change).
\item {\em Surprising}; i.e. we want the planners to (occasionally) tell us things we do not know already (otherwise,
there would be no point to using the planner).
\ei 
The rest of this paper is describes our data, our planners, and the experiment that
...
\caption{Percent frequency for how often certain feature was changed bu a plan.}\label{fig:changed}
\end{figure*}
...
\bi
\item
``Fentle slopes'' the lead to small changes
in  continuous space;
\item
...
Such a plan was seen in the {\em lucene} results of \fig{changed}. Recall the standard advice for
OO systems: build classes that are internally cohesive with low coupling to other parts of the system.  
\subsubsection{Summary}
In summary, we  have two reasons to endorse XTREE. Firstly, it is usually far more effective than other
methods. Secondly, even when we jump from continuous to discrete features, XTREE's plans consistently
use just a small portion of the features. 

In terms of the assumptions that motivated our different methods, we note that 
{\em Assumption4} seems most powerful; i.e. for the purposes of planning,
it is better to reflect on the target class.  
...
\subsection{  Sampling bias} threatens any data mining experiment; i.e., what matters
...
\subsection{  Evaluation bias} Another threat to validity of this work is our use
...
As shown in \fig{coc}, those results endorsed the conclusions of the rest
of this paper; i.e. that XTREE's supervised methods are best for generating plans on how to change
example projects.
...
of \tion{trust}, these methods may not be {\em trust-increasing} those algorithms make no attempt
