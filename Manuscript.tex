...
\be
\item
The  XTREES planner, which has not been previously pubslished. For demonstration purposes, we show XTREES
...
Our answer is ``yess'', if those changes select for the {\em region of competency} of the predictor. When we assess
...
a model learned from the training data applies to the test data. Such experiments return some performance value, from
which we can say that if ever any new data falls into the combined region of the training+test cloud, then we should expect
at least that level of performance.

A corollary of the above is that if we are changing examples, and making predictions about those changed examples,
then:
\bi
...
\item More specifically: predictions  about changes that  fall within the space of the training+test data, will be at least
as accurate as the 
\ei  
