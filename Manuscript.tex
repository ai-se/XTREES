...
\ee
In all, this paper explores and comparatively assesses four methods for learning planners (and XTREES is Method\#4).
As an historical note,  Method\#2 was previously proposed in one of our prior publications~\cite{me12c}. As shown below,
that method is a comparatively far less effective than XTREES. Hence, we   recant that prior work and, based on the results of this
paper, we now endorse  XTREES.
...
\begin{figure}[!t]
  \includegraphics[width=2in,bb=0 0 100 100]{figs/aa.eps} 
...
XTREES performs well in terms of  effectiveness. 
...
\ei
In all these results,  with one exception, XTREES always is the top-ranked method and with a median improvement at least 10\% lower (i.e. better) than the next ranked method. The  exception is SQL which (as shown in \fig{conf}), seems to defy all methods for optimization
so XTREES can only reduce the expected values of the runtimes by 1\%.
...
probably explains why CD+FS performs so poorly for {\em BDBJ} in \fig{conf}). Finally, Method4 (XTREES) rarely fixates
...
Method3 (BIC) make the most changes while XTREES and CD+FS make the fewest.
