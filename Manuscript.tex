...
automatic methods to find plans from
  $N$ examples divided  into  clumps containing at most \mbox{ $\alpha=\sqrt{N}$}
  examples. 
...
For details on the Jurecko data, see  \fig{ck} and \fig{jd}. 
  ...
  four planning methods (to learn different plans). Next, using the train set,
  we apply the plans and ask the quality predictor to assess the results.
...
This paper  explores six Siegmund configuration data sets:  Berkeley DB (Java and C versions), Apache, SQLite, LLVM, and
  ...
  Regressor\footnote{From the SciKit learn kit~\cite{Pedregosa2012}.}   on one half, then applied to the other. This  achieved nearly perfect scores of \[s=\{99.9, 99.8, 99.4, 99.1, 96.1\}\] 
...
our plans. (Aside: if the reader doubts that such high scores are achievable, we note that these scores are consistent with those achieved by predictors built by Siegmund et al.~\cite{sven12}.)
 ...
 It proved to be somewhat more complicated to commission the Jureczko data sets for this study.
 For the Jureczko OO data sets, we found:
\bi
\item
Most quality predictors built from this data were far from perfect;
\item 
